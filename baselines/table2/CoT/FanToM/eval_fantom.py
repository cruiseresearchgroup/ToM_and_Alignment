...
class FantomEvalAgent():
    ...
    def evaluate_mc_belief_q(self, qa, model_response):
        ...
        #     return True
        # print(response==str(qa['correct_answer']))
        if response==str(qa['correct_answer']):
            return True
        else:
            return False
    ...
    def evaluate_fact_q(self, qa, model_response):
        result = self.compute_f1(str(qa['correct_answer']).lower(), model_response.lower())
        return result
    ...
    def get_last_savepoint(self):
        ...
        if os.path.exists(model_responses_filename_path):
            ...
            try:
                ...
                df = pd.read_json(model_responses_filename_path)
            if len(df) > 0:
